# ansible-lab-infra
Using vagrant + ansible to generate a lab infra with one controller node and 4 managed hosts

To generate the infra, clone the repo
```
...
org_id:
```
Finally, put your vault password inside `vault_pass` file
You can now run the generation script

```
chmod +x generate-ansible-infra.sh
./generate-ansible-infra.sh
```
...
FQDNs will be added to your /etc/hosts so you can ssh using 
